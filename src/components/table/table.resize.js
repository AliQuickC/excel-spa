...
export function resizeHandler($root, event) {
  return new Promise(resolve => {
    ...
    document.onmousemove = e => { // двигаем маркет, для изменения размера столбца/строки
    ...
    document.onmouseup = () => { // фиксируем новый размер столбца/строки
      ...
      if (type === 'col') { // если тянем колонку
        ...
        $root.findAll(`[data-col="${$parent.data.col}"]`) // находим на странице все ячейки,
        //                                          // где дата атрибут равет номеру меняемой строки
